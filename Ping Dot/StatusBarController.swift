...
final class StatusBarController {
    ...
    private func buildMenu() {
        ...
        menu.addItem(NSMenuItem(
            title: "Quit Ping Dot", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"
        ))
    ...
    private func updateStatusLine(status: PingService.Status?) {
        guard let mi = item.menu?.item(withTag: Tag.statusLine.rawValue) else { return }
        switch status {
        case .ok:  mi.title = "● \(settings.host)  —  reachable"
        case .nok: mi.title = "● \(settings.host)  —  unreachable"
        case nil:  mi.title = "● \(settings.host)  —  checking…"
        }
    }
    ...
    private func makeIcon(status: PingService.Status?) -> NSImage {
        ...
        let image = NSImage(size: NSSize(width: canvas, height: canvas), flipped: false) { _ in
            let path = NSBezierPath(ovalIn: NSRect(x: o, y: o, width: d, height: d))

            switch status {
            case .ok:
                NSColor.white.setFill()
                path.fill()
                // subtle outline for light menu bar visibility
    ...
    @objc private func openSettings() {
        if let existing = settingsController, existing.window?.isVisible == true {
        ...
        controller.onSave = { [weak self] newSettings in
            ...
            newSettings.save()
            self.ping.restart(host: newSettings.host, interval: newSettings.interval)
